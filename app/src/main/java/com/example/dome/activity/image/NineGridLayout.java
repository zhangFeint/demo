...
public abstract class NineGridLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mSingleWidth = (int) ((mTotalWidth - mSpacing * (largest - 1)) / largest);
        if (mIsFirst) {
     ...
     * @param largest
     */
    public void setColumnColumns(int largest) {
    ...
    private RatioImageView createImageView(final int i, final String url) {
        ...
        imageView.setOnLongClickListener(new OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                onLongClickImage(i, url, mUrlList);
                return true;
            }
        });
        return imageView;
    ...
    private void layoutImageView(RatioImageView imageView, int i, String url, boolean showNumFlag) {
        final int singleWidth = (int) ((mTotalWidth - mSpacing * (largest - 1)) / largest);
    ...
    private void generateChildrenLayout(int length) {
        if (length <= largest) {
            ...
            mColumns = largest;
            if (mIsShowAll) {
                mRows = length / largest;
                int b = length % largest;
                if (b > 0) {
                    mRows++;
                }
            } else {
...
//        params.width = width;
    ...
    protected abstract void onLongClickImage(int position, String url, List<String> urlList);
}
